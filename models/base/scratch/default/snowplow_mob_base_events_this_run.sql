...
{%- set lower_limit, upper_limit = snowplow_utils.return_limits_from_model(ref('snowplow_mob_base_events_this_run_limits'),
                                                                          'lower_limit',
                                                                          'upper_limit') %}
...
with events_this_run AS (
  ...
  inner join {{ ref('snowplow_mob_base_session_context') }} sc
  on e.event_id = sc.root_id
  and e.collector_tstamp = sc.root_tstamp
  inner join {{ ref('snowplow_mob_base_sessions_this_run') }} str
...
{% if var('snowplow__enable_screen_context') %}
  left join {{ ref('snowplow_mob_base_screen_context') }} sc
  on e.event_id = sc.root_id
  and e.collector_tstamp = sc.root_tstamp
{% endif %}

{% if var('snowplow__enable_mobile_context') %}
  left join {{ ref('snowplow_mob_base_mobile_context') }} mc
  on e.event_id = mc.root_id
  and e.collector_tstamp = mc.root_tstamp
{% endif %}

{% if var('snowplow__enable_geolocation_context') %}
  left join {{ ref('snowplow_mob_base_geo_context') }} gc
  on e.event_id = gc.root_id
  and e.collector_tstamp = gc.root_tstamp
{% endif %}

{% if var('snowplow__enable_application_context') %}
  left join {{ ref('snowplow_mob_base_app_context') }} ac
