...
for db in ${DATABASES[@]}; do
  ...
  echo "Snowplow mobile integration tests: Run 1: standard modules"

  eval "dbt run --target $db --full-refresh --vars '{snowplow__allow_refresh: true, snowplow__backfill_limit_days: 60}'" || exit 1;

  echo "Snowplow mobile integration tests: Run 2: standard modules"

  eval "dbt run --target $db" || exit 1;

  echo "Snowplow mobile integration tests: Run 3: Partial backfill of custom module + standard modules"
  ...
  eval "dbt run --target $db --vars '{snowplow__enable_custom_example: true, snowplow__backfill_limit_days: 60}'" || exit 1;

  echo "Snowplow mobile integration tests: Run 4: Partial backfill of custom module only"

  eval "dbt run --models +snowplow_mobile_ses_goals --target $db --vars 'snowplow__enable_custom_example: true'" || exit 1;

  for i in {5..6}
  do
    echo "Snowplow mobile integration tests: Run $i/6: Standard increment - all modules"
