...
{% set lower_limit, upper_limit, session_lookback_limit, _ = snowplow_utils.return_base_new_event_limits(ref('snowplow_mob_base_new_event_limits')) %}
{% set is_run_with_new_events = snowplow_utils.is_run_with_new_events('snowplow_mob') %}
{% set session_id = snowplow_mob.get_session_id_path_sql(relation_alias='e') %}
